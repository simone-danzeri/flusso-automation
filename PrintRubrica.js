function printRubrica() {
  ...
  const numberOfContactsToPrint = input() ;

  if(numberOfContactsToPrint !== 0){
        // Colonne dati
      const sourceColumnName = 2; 
      ...
      if(sourceDataPrivacyCalcolo.length - 1 > 0){
        while(count < numberOfContactsToPrint){
          for(let i = 0; i < sourceDataPrivacyCalcolo.length -1 && count < numberOfContactsToPrint; i++) {
            if (sourceDataPrivacyCalcolo[i][0] === 0) {
              ...
              const name = details !== "" ? fileName + "_" + details + "_" + sourceDataNameCalcolo[i][0] : fileName + "_" + sourceDataNameCalcolo[i][0];
          ...
          for(let i = 0; i < sourceDataPrivacyIntegra.length - 1 && count < numberOfContactsToPrint; i++) {
            if(sourceDataNameIntegra[i][0] === "Z-INTEGRA"){
              count = numberOfContactsToPrint;
            } else {
                if (sourceDataPrivacyIntegra[i][0] === 0) {
                  // Normalizza il numero
                  const details = sourceSheetIntegra.getRange("A2").getValue().replace(/\s+/g, '').replace(/,/g,"_");
                  const name = details !== "" ? fileName + "_" + details + "_" + sourceDataNameIntegra[i][0] : fileName + "_" + sourceDataNameIntegra[i][0];
    ...
    if(datas.length > 0) {
      ...
      newIndSheet.getRange(1, 2).setValue("NOME");
      newIndSheet.getRange(1, 3).setValue("LINK");
      ...
      sourceSheetDatabase.getRange(databaseLastRow, 1, filteredRubrica.length, 2).setValues(filteredRubrica);
        ...
        ui.alert("Attenzione", "Non hai selezionato contatti da integrare", ui.ButtonSet.OK);
    }
  } else {
      const ui = SpreadsheetApp.getUi();
      ui.alert("Attenzione", "Qualcosa e' andato storto (per colpa tua)", ui.ButtonSet.OK);
  }
